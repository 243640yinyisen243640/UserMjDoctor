...
import com.xy.xydoctor.adapter.SportAdapter;
import com.xy.xydoctor.bean.SportBean;
 ...
 * 创建日期: 2019/3/6 17:06
 */
public class HealthRecordSportListActivity extends BaseHideLineActivity {
    ...
    ListView lvSport;
    @BindView(R.id.srl_sport)
    SmartRefreshLayout srlSport;
    private RotateAnimation showArrowAnima;
    private RotateAnimation dismissArrowAnima;
    private int pageIndex = 1;//当前获取的是第几页的数据
    private List<SportBean> list = new ArrayList<>();//ListView显示的数据
    private List<SportBean> tempList;//用于临时保存ListView显示的数据
    ...
    private void initValue(String beginTime, String endTime) {
        ...
        RxHttp.postForm(XyUrl.GET_SPORT_LIST)
                .addAll(map)
                .asResponseList(SportBean.class)
                .to(RxLife.toMain(this))
                .subscribe(new Consumer<List<SportBean>>() {
                    @Override
                    public void accept(List<SportBean> myTreatPlanBeans) throws Exception {
                        tempList = myTreatPlanBeans;
                        //少于10条,将不会再次触发加载更多事件
                        if (tempList.size() < 10) {
                            srlSport.finishLoadMoreWithNoMoreData();
                        } else {
                        ...
                        if (pageIndex == 1) {
                            ...
                            adapter = new SportAdapter(Utils.getApp(), R.layout.item_health_record_4, list);
    ...
    private void initReFresh() {
        srlSport.setEnableAutoLoadMore(true);//开启自动加载功能(非必须）
        //下拉刷新
    ...
    protected int getLayoutId() {
        return R.layout.activity_health_record_blood_sport_list;
    }

    @Override
    protected void init(Bundle savedInstanceState) {
        //setTitle("记录运动");
        tvTopTitle.setText("运动记录");
        hideTitleBar();
        initReFresh();
        initValue(beginTime, endTime);
        initPopu();
