...
import com.xy.xydoctor.adapter.SaccharifyAdapter;
 ...
 * 创建日期: 2019/3/6 16:59
 */
public class HealthRecordSaccharifyListActivity extends BaseHideLineActivity {
    ...
    TextView tvEndTime;
    //popu结束
    @BindView(R.id.lv_saccharify)
    ListView lvSaccharify;
    @BindView(R.id.srl_saccharify)
    SmartRefreshLayout srlSaccharify;
    private RotateAnimation showArrowAnima;
    private RotateAnimation dismissArrowAnima;
    private int pageIndex = 1;//当前获取的是第几页的数据
    private List<SaccharifyBean> list = new ArrayList<>();//ListView显示的数据
    private List<SaccharifyBean> tempList;//用于临时保存ListView显示的数据
    private SaccharifyAdapter adapter;
    ...
    private void initValue(String beginTime, String endTime) {
        ...
        RxHttp.postForm(XyUrl.GET_SACCHARIFY_LIST)
                .addAll(map)
                .asResponseList(SaccharifyBean.class)
                .to(RxLife.toMain(this))
                .subscribe(new Consumer<List<SaccharifyBean>>() {
                    @Override
                    public void accept(List<SaccharifyBean> myTreatPlanBeans) throws Exception {
                        tempList = myTreatPlanBeans;
                        //少于10条,将不会再次触发加载更多事件
                        if (tempList.size() < 10) {
                            srlSaccharify.finishLoadMoreWithNoMoreData();
                        } else {
                            srlSaccharify.finishLoadMore();
                        }
                        //设置数据
                        if (pageIndex == 1) {
                            //
                            ...
                            adapter = new SaccharifyAdapter(Utils.getApp(), R.layout.item_health_record_4, list);
                            lvSaccharify.setAdapter(adapter);
                        } else {
                            list.addAll(tempList);
    ...
    private void initReFresh() {
        srlSaccharify.setEnableAutoLoadMore(true);//开启自动加载功能(非必须）
        //下拉刷新
        srlSaccharify.setOnRefreshListener(new OnRefreshListener() {
            @Override
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                pageIndex = 1;
                initValue(beginTime, endTime);
                srlSaccharify.finishRefresh();
                refreshLayout.setNoMoreData(false);//恢复没有更多数据的原始状态
            }
        });
        //上拉加载
        srlSaccharify.setOnLoadMoreListener(new OnLoadMoreListener() {
            @Override
    ...
    protected int getLayoutId() {
        return R.layout.activity_health_record_saccharify_list;
    }

    @Override
    protected void init(Bundle savedInstanceState) {
        //setTitle("记录糖化血红蛋白");
        tvTopTitle.setText("糖化血红蛋白记录");
        hideTitleBar();
        initReFresh();
        initValue(beginTime, endTime);
        initPopu();
